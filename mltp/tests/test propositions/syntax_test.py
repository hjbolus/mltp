...
def test_repr(debug=False):
    ...
    assert str(Formula('~', Formula('&', Formula('p'), Formula('q7')))) == \
           '~(p&q7)'
    if debug:
        print("Testing representation of formula '((p->q)->(~q->~p))'")
    assert str(Formula('->', Formula('->', Formula('p'), Formula('q')), Formula('->', Formula('~', Formula('q')), Formula('~', Formula('p'))))) == '((p->q)->(~q->~p))'
...
def test_substitute_operators(debug=False):
    ...
    tests = [ ("v",                 {},                            "v"),
              ...
              ("(x&(y|z))",         {"&":"(q|p)"},                 "((y|z)|x)"),
              ("((x&y)&~z)",        {"&":"~(~p|~q)"},              "~(~~(~x|~y)|~~z)"),
              ("((x+y)+x)",         {"+":"~((p&q)|(~p&~q))"},      "~((~((x&y)|(~x&~y))&x)|(~~((x&y)|(~x&~y))&~x))")]
    for f, d, r in tests:
        if debug:
        ...
        assert a == r, "Incorrect answer:"+a+", the answer is " + r
