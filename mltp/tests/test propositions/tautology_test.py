...
from proofs_test import offending_line
...
def test_prove_in_model(debug=False):
    for (f, m, a, cp) in [ ('x', {'x':True}, ['x'], ''),
                           ...
                           ('~~~~y7', {'y7':True}, ['y7'], ''),
                           ('((p1->p2)->(p3->p4))', {'p1':True, 'p2':True, 'p3':True, 'p4':True}, ['p1','p2','p3','p4'], ''),
                           ('(p->q)', {'p':False, 'q':True}, ['~p','q'], ''),
                           ('(p->q)', {'p':False, 'q':False}, ['~p', '~q'], ''),
                           ('~~~~y7', {'y7':False}, ['~y7'], '~'),
                           ('(p->~p)', {'p':False}, ['~p'], ''),
                           ('~(~p->~q)', {'p':True, 'q':True}, ['p','q'], '~'),
                           ('(p->q)', {'p':True, 'q':True}, ['p','q'], ''),# success
                           ('(~(~x->~~y)->(z->(~x->~~~z)))', {'z':True, 'x':False, 'y':False}, ['~x','~y','z'], '~'), #fail
                           ('((p1->p2)->(p3->p4))', {'p1':True, 'p2':False, 'p3':True, 'p4':False}, ['p1','~p2','p3','~p4'], ''), #fail
                           ('((p1->p2)->(p3->p4))', {'p1':True, 'p2':True, 'p3':True, 'p4':False}, ['p1','p2','p3','~p4'], '~'), #fail
                           ('~(~p->~q)', {'p':False, 'q':True}, ['~p','q'], ''), #fail
                           ('(p->~p)', {'p':True}, ['p'], '~'), #fail
                           ('(p->q)', {'p':True, 'q':False}, ['p','~q'], '~') #fail
                           ]:
...
def test_prove_tautology(debug=False):
    for f, m in [ ('(p->p)', {'p':True}),
        ...
        assert p.statement.assumptions == tuple(formulas_capturing_model(m))
        assert p.rules == AXIOMATIC_SYSTEM
        assert p.is_valid(), offending_line(p)
