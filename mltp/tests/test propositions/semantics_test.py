...
def test_all_models(debug=False):
    ...
    variables3 = ('q', 'p')
    models3 = [{'q': False, 'p': False},
               {'q': False, 'p': True},
               {'q': True,  'p': False},
               {'q': True,  'p': True} ]
    for variables,models in [(variables1, models1),
                             (variables2, models2),
                             (variables3, models3)]:
        if debug:
            print('Testing all models over', variables)
            print('\n')
        print(list(all_models(tuple(variables))))
        print(models)
        print('\n')
...
def __test_print_truth_table(infixes, tables, debug):
    ...
    for infix,table in zip(infixes, tables):
        ...
        if debug:
            ...
            print ('Expected:\n' + table)
        import re
        assert re.sub('[ -]+', ' ', capturer.captured) == re.sub('[ -]+', ' ', table)

def test_is_tautology(debug=False):
    for infix,answer in [['~(p&q7)',False], ['(x|~x)', True], ['(p->q)', False],
                         ['(p->p)', True], ['(F|T)', True],
                         ['((y1|~y1)&T)', True], ['((T&T)|F)', True],
                         ['F', False], ['x', False], ['~y', False],
                         ['((x->y)&((y->z)&(x&~z)))', False],
                         ['~((x->y)&((y->z)&(x&~z)))', True]]:
        formula = Formula.parse(infix)
        if debug:
...
def test_is_contradiction(debug=False):
    for infix,answer in [['~(p&q7)', False], ['~(x|~x)', True],
                         ...
                         ['((T&T)|F)', False], ['F', True], ['x', False],
                         ['~y', False], ['((x->y)&((y->z)&(x&~z)))', True]]:
        formula = Formula.parse(infix)
        if debug:
...
def test_is_satisfiable(debug=False):
    for infix,answer in [['~(p&q7)', True], ['~(x|~x)', False],
                         ...
                         ['((T&T)|F)', True],  ['F', False], ['x', True],
                         ['~y', True], ['((x->y)&((y->z)&(x&~z)))', False]]:
        formula = Formula.parse(infix)
        if debug:
...
def __test_synthesize_clause(clause_synthesizer, for_model, debug):
    ...
    for all_models in [all_models1, all_models2, all_models3]:
        for idx in range(len(all_models)):
            ...
            assert isinstance(f, Formula), 'Expected a formula, got ' + str(f) + ' of type ' + str(type(f))
            if for_model:
...
def is_conjunctive_clause(f):
    if is_variable(f.root) or (f.root == '~' and is_variable(f.first.root)):
        return True
    return f.root == '&' and is_conjunctive_clause(f.first) and \
           is_conjunctive_clause(f.second)

def is_disjunctive_clause(f):
    if is_variable(f.root) or (f.root == '~' and is_variable(f.first.root)):
        return True
    return f.root == '|' and is_disjunctive_clause(f.first) and \
           is_disjunctive_clause(f.second)
    ...
    for all_variables, all_models,value_lists in [
        ...
        for all_values in value_lists:
            ...
            assert type(formula) is Formula, \
                   'Expected a formula, got ' + str(formula) + ' of type ' + type(formula)
            if dnf:
...
def is_cnf(formula):
    return is_disjunctive_clause(formula) or \
           (formula.root == '&' and is_cnf(formula.first) and
            is_cnf(formula.second))
