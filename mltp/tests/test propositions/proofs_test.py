...
def test_merge_specialization_maps(debug=False):
    for d1, d2, d in [
        ...
        assert dd == ({v: Formula.parse(d[v]) for v in d}
                      if d is not None else None), "got " + str(dd)
...
specializations = [
      ...
      ['(x&y)', '(F&F)', {'x':'F', 'y':'F'}],
      ['(x&y)', '(F&~T)', {'x':'F', 'y':'~T'}],
      ['(~p->~(q|T))', '(~(x|y)->~((z&(w->~z))|T))', {'p':'(x|y)', 'q':'(z&(w->~z))'}],
      ['~F', '~x', None],
      ['~F', '~T', None],
      ['F', '(x|y)', None],
      ['(x&x)', '(F&T)', None],
...
def test_is_line_valid(debug=False):
    ...
    if debug:
        print("\nChecking proof line validity in proof of", lemma,
...
#    from proofs import _inline_proof_once
    from mltp.propositions.some_proofs import prove_and_commutativity
    ...
    if debug:
        print('Testing _inline_proof_once (test 1). In main proof:\n',
              proof, "Replacing line", line_number,
              'with the proof of following lemma proof:\n',
              str(lemma2_proof))
    inlined_proof = _inline_proof_once(proof, line_number, lemma2_proof)
    if debug:
        print("\nGot:", inlined_proof)
    ...
    line_number = first_use_of_rule(proof, rule)
    if debug:
        print('Testing _inline_proof_once (test 2). In main proof:\n',
              proof, "Replacing line", line_number,
              'with the proof of following lemma proof:\n',
              str(lemma2_proof))
    inlined_proof = _inline_proof_once(proof, line_number, lemma2_proof)
    ...
    # Accept both the implementation from the guidelines (resulting in 3 uses
    # here) and a more efficient implementation that doesn't duplicate
    # assumption lines (resulting in 2 uses here).
    assert uses_before_loop in {2, 3}
    for count in range(uses_before_loop):
        # Test inlining lemma1_proof into result of previous inlining
        proof = inlined_proof
        assert proof.is_valid(), offending_line(proof)
        rule = lemma1_proof.statement
        assert uses_of_rule(proof, rule) == uses_before_loop - count
        line_number = first_use_of_rule(proof, rule)
        if debug:
            print('Testing _inline_proof_once (test ' + str(3+count) +
                  '). In main proof:\n', proof, "Replacing line", line_number,
                  'with the proof of following lemma proof:\n',
                  str(lemma1_proof))
        inlined_proof = _inline_proof_once(proof, line_number, lemma1_proof)
        if debug:
            print("\nGot:", inlined_proof)
        assert inlined_proof.statement == proof.statement
    ...
    if debug:
        print('Testing _inline_proof_once (final). In main proof:\n',
              proof, "Replacing line", line_number,
              'with the proof of following lemma proof:\n',
              str(lem_proof))
    inlined_proof = _inline_proof_once(proof, line_number, lem_proof)
    if debug:
        print("\nGot:", inlined_proof)
    assert inlined_proof.statement == proof.statement
...
def test_inline_proof(debug=False):
    ...
    assert inlined_lemma.rules == \
           lemma1_proof.rules.union(lemma2_proof.rules) - \
           {lemma1_proof.statement}
    ...
    assert inlined_proof.statement == proof.statement
    assert inlined_proof.rules == \
           inlined_lemma.rules.union(inlined_proof.rules) - \
           {lemma2_proof.statement}
    assert inlined_proof.is_valid(), offending_line(inlined_proof)
