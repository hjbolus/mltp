...
def test_term_substitute(debug=False):
    ...
    for s,expected in [['x', 'g(1)'], ['0', '0'], ['f(x)', 'f(g(1))'],
                       ['s(s(x))', 's(s(g(1)))'],
    ...
    substitution_map = {'cd':Term.parse('g(f(xy))'),
                        'xy':Term.parse('f(cd,xy)'), '_':Term.parse('d')}
    result = Term.parse('h(cd,f(xy,_))').substitute(substitution_map, {'y'})
    if debug:
        print('Substituting', substitution_map, " in 'h(c,f(x, _))' gives",
              result)
    assert str(result) == 'h(g(f(xy)),f(f(cd,xy),d))'
...
def test_formula_parse_prefix(debug=False):
    for s in ['0=_', 'R(x)|Q(y))', 'Q(y))', '0=0&1=1)', '1=1)', 'Q()&R(x))',
              'R(0)&0=0)|Ex[Q(x)])', '0=0)|Ex[Q(x)])', 'R(x,y)',
              'Q(x)])', 'PLUS(s(x),y,s(plus(x,y)))', '~R(x)', 'R(x))',
              'plus(_,y)=plus(y,x)]]', 'R(x)', 'Q(x)]', '~Q(x)]', '~x=f(y)',
              'R(x8,x7,c)]', 'R(x,y)]]', 'Ay[R(x,y)]]]', 'Ax8[R(x8,x7,c)]', '~Ax[~Q(x)]',
              'Ex[Ay[R(x,y)]]', 'Ex[Q(x)])', 'Ax[~Q(x)]', '~~Ax[~Q(x)]',
              'Ay[plus(x,y)=plus(y,x)]]', 'Ax[Ay[plus(x,y)=plus(y,x)]]',
              '(Q()&R(x))', '(0=0&1=1)', '((R(0)&0=0)|Ex[Q(x)])', '(R(0)&0=0)|Ex[Q(x)])',
              '(Q()->(P()->Q()))', '((~Q()->~P())->(P()->Q()))', '(R(x)|Q(y))',
              '((P()->(Q()->R()))->((P()->Q())->(P()->R())))']:
        if debug:
            print('Parsing a prefix of', s, 'as a predicate-logic formula...')
        formula, remainder = Formula._parse_prefix(s)
        if debug:
...
def test_formula_parse(debug=False):
    for s in ['_=0', '~R(x)', '~~Ax[~Q(x)]', 'Ex[Ay[R(x,y)]]',
              'Ax[Ay[plus(x,y)=plus(y,_)]]',
              'Ax8[R(x8,x7,c)]', 'R(x,y)', 'PLUS(s(x),y,s(plus(x,y)))',
              '~x=f(y)', '((R(0)&0=0)|Ex[Q(x)])',
              '(Q()&R(x))', '(R(x)|Q(y))', '(0=0&1=1)',
              '(Q()->(P()->Q()))', '((~Q()->~P())->(P()->Q()))',
              '((P()->(Q()->R()))->((P()->Q())->(P()->R())))']:
        if debug:
            print('Parsing', s, 'as a predicate-logic formula...')
...
def test_formula_constants(debug=False):
    for s,expected_constants in [
            ['x=x', set()], ['x=0', {'0'}], ['c34=_', {'c34', '_'}],
            ['R(s(s(c)))', {'c'}], ['Ax[~x=1]', {'1'}],
            ['Ax[(x=2|x=1)]', {'1', '2'}], ['Ey[R(s(s(d)))]', {'d'}],
            ['Ax[Ez[f(x,g(y,x,0),12)=d]]', {'0', '12', 'd'}]]:
...
def test_formula_variables(debug=False):
    for s,expected_variables in [
            ['0=0', set()], ['x=0', {'x'}], ['R(s(s(x12)))', {'x12'}],
            ['Ax[~x=1]', {'x'}], ['Ax[(x=2|x=1)]', {'x'}],
            ['Ey[R(s(s(x)))]', {'x', 'y'}], ['Ex12[R(s(s(x12)))]', {'x12'}],
            ['(Ex[x=0]|x=1)', {'x'}], ['(x=13&Ex[x=c])', {'x'}],
        ...
        if debug:
            print('The variables in', s, 'are', variables)
        assert variables == expected_variables, f'Formula was {Formula.parse(s)}, we expected {expected_variables}, you gave {variables}'

def test_free_variables(debug=False):
    for s,expected_variables in [
            ['0=0', set()], ['x=0', {'x'}], ['R(s(s(x12)))', {'x12'}],
            ['Ax[~x=1]', set()], ['Ax[(x=2|x=1)]', set()],
            ['Ey[R(s(s(x)))]', {'x'}], ['Ex12[R(s(s(x12)))]', set()],
            ['(Ex[x=0]|x=1)', {'x'}], ['(x=13&Ex[x=c])', {'x'}],
...
def test_relations(debug=False):
    for s,expected in [
        ...
        assert relations == expected, f'Formula was {Formula.parse(s)}, we expected {relations}, you gave {Formula.parse(s).relations()}'
...
def test_formula_substitute(debug=False):
    for s,substitution,expected in [
            ...
            ('(xx=xx|Exx[R(xx,y)])', {'xx':'z'}, '(z=z|Exx[R(xx,y)])'),
            ('R(c,y)', {'c':'f(0)'}, 'R(f(0),y)'),
            ('(x=x|c=c)', {'c':'0'}, '(x=x|0=0)'),
            ('Ax[R(x,cc)]', {'cc':'z'}, 'Ax[R(x,z)]'),
            ('(c=c|Ex[R(c,y)])', {'c':'z'}, '(z=z|Ex[R(z,y)])'),
            ('R(c,d)', {'c':'d','d':'c'}, 'R(d,c)'),
            ('Q(x,c)', {'x':'f(c,d)','c':'f(1,x)'}, 'Q(f(c,d),f(1,x))'),
            ('(Ex[x=y]&Ey[x=w])', {'x':'z'}, '(Ex[x=y]&Ey[z=w])')]:
        formula = Formula.parse(s)
    ...
    for s,substitution,forbidden_variables,variable_name in [
        ...
        substitution_map = {v: Term.parse(substitution[v])
                            for v in substitution.keys()}
        if debug:
            print('Substituting', substitution_map, 'in', formula, 'with forbidden variables', forbidden_variables )
        try:
            result = str(formula.substitute(substitution_map,
                                            forbidden_variables))
...
def test_propositional_skeleton(debug=False):
    from mltp.logic_utils import fresh_variable_name_generator
    ...
    for s,expected,expected_map in [
        ...
        assert (str(skeleton), substitution_map) == (expected, expected_map), print(f'got {skeleton, substitution_map}, expected {expected, expected_map}')
