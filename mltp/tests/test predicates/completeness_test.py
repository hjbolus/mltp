...
from mltp.logic_utils import fresh_constant_name_generator
from mltp.propositions import is_tautology as is_propositional_tautology
...
def _test_is_closed(test_primitively=False, test_universally=False,
    ...
    def _test_closures(sentences, primitively, universally, existentially):
        ...
        if test_primitively:
            assert is_primitively_closed(sentences) == primitively, print(f'expected {not primitively}, got {primitively}')
        if test_universally:
            assert is_universally_closed(sentences) == universally, print(f'expected {not universally}, got {universally}')
        if test_existentially:
            assert is_existentially_closed(sentences) == existentially, print(f'expected {not existentially}, got {existentially}')
    ...
    for six_element_group_primitives in [
        ...
        for commutativity in [
            ...
            if type(result) is Model:
                ...
                assert is_propositional_tautology(
                    Formula('~', result.conclusion).propositional_skeleton()[0]), print(f'{result.conclusion} is not a contradiction')
                if debug:
...
def test_eliminate_universal_instantiation_assumption(debug=False):
    ...
    assert is_propositional_tautology(
        Formula('~', eliminated.conclusion).propositional_skeleton()[0]), print(f'{Formula("~", eliminated.conclusion)} is not a tautology')
    if debug:
        print('Verifying returned proof (' +
...
#         GROUP_AXIOMS, prove_group_unique_zero
    from some_proofs_test import test_prove_group_unique_zero
...
#    from logic_utils import fresh_constant_name_generator
    ...
    assert augmented1 == {ZERO_AXIOM, NEGATION_AXIOM, ASSOCIATIVITY_AXIOM,
                          NONCOMMUTATIVITY,
                          Formula.parse('Ey[Ez[(Plus(z,e7,y)&~Plus(z,y,e7))]]')}
    ...
    assert augmented2 == {ZERO_AXIOM, NEGATION_AXIOM, ASSOCIATIVITY_AXIOM,
                          NONCOMMUTATIVITY,
                          Formula.parse('Ey[Ez[(Plus(z,e7,y)&~Plus(z,y,e7))]]'),
                          Formula.parse('Ez[(Plus(z,e7,e8)&~Plus(z,e8,e7))]')}
    ...
    assert augmented3 == {ZERO_AXIOM, NEGATION_AXIOM, ASSOCIATIVITY_AXIOM,
                          ...
                          Formula.parse('Ey[Ez[(Plus(z,e7,y)&~Plus(z,y,e7))]]'),
                          Formula.parse('Ez[(Plus(z,e7,e8)&~Plus(z,e8,e7))]'),
                          Formula.parse('(Plus(e9,e7,e8)&~Plus(e9,e8,e7))')}
