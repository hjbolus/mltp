...
#         _pull_out_quantifications_from_left_across_binary_operator
    ...
    for formula,expected in [
        ...
        if debug:
            print('... got', result)
        assert str(result) == expected, print(result, expected)
        assert proof.assumptions == \
            Prover.AXIOMS.union(ADDITIONAL_QUANTIFICATION_AXIOMS)
        assert proof.conclusion == equivalence_of(formula, result), print(proof.conclusion,'\t\t', equivalence_of(formula, result))
        assert proof.is_valid()
...
#         _pull_out_quantifications_across_binary_operator
    ...
    for formula,expected in [
        ...
        assert proof.assumptions == \
            Prover.AXIOMS.union(ADDITIONAL_QUANTIFICATION_AXIOMS)
        assert proof.conclusion == equivalence_of(formula, result), print(f'expected {equivalence_of(formula, result)}, got {proof.conclusion}')
        assert proof.is_valid()
...
#         _to_prenex_normal_form_from_uniquely_named_variables
    ...
    for formula,pnf in [
        ...
        ('~(z=x|Au[(Ezz[(zz=u&Aw[w=zz])]->Auu[uu=y])])', 'Eu[Ezz[Aw[Euu[~(z=x|((zz=u&w=zz)->uu=y))]]]]')]:
        formula = Formula.parse(formula)
        if debug:
            print('Testing '
