...
This textbook provides code skeletons for functions that, when implemented, automate tasks in formal logic. I worked on them over 2023-2024, and the result is a Python package for classical first order and propositional logic. This repository is the first (and only, as of September 2025) complete set of solutions available. I have added some utilities without changing the existing ones, so everything in the documentation above is still accurate, with the addition of, for example, predicates.operators and cleaning utilities for proofs.

# First order logic
## Formulas
Formulas are stored as objects of class `Formula`, which are composed of operators, subformulas, and terms (`Term` objects) in an expression tree or directed-acyclic graph. Terms are either constants, variables, or functions. Terms and formulas can be parsed from strings into objects of their respective type using the `parse()` methods of their respective classes. For example, after installing with `pip install mltp`:

`In [1]: import mltp.predicates as *`  

`In [2]: formula = Formula.parse('Ax[(Man(x)->Mortal(x))]')`  

`In [3]: print(f'{formula} is composed of the quantifier {formula.root}, bound variable {formula.variable}, and statement {formula.statement}. The statement is composed of subformulas {formula.statement.first} and {formula.statement.second}, with the operator {formula.statement.root}')`  

`Out [3]: Ax[(Man(x)->Mortal(x))] is composed of the quantifier A, bound variable x, and statement (Man(x)->Mortal(x)). The statement is composed of subformulas Man(x) and Mortal(x), with the operator ->`  
...
`In [4]: schema = Schema(Formula.parse('(c=d->(R(c)->R(d)))'), {'R', 'c', 'd'})`  

`In [5]: schema.instantiate({'R': Formula.parse('_=z'), 'c': Term('x'), 'd': Term('y')})`  
...
`In [6]: proof = prove_syllogism() # a proof stored in a function (for nefarious reasons)`  

`In [7]: proof`  

`Out [7]: Proof of Mortal(aristotle) from assumptions/axioms:`  
...
`In [9]: remove_assumption(proof, formula)`  

`Out [9]: Proof of (Ax[(Man(x)->Mortal(x))]->Mortal(aristotle)) from assumptions/axioms:`  
`  Schema: Man(aristotle) [templates: none]`  
`  ...`  
...
`In [10]: formula, proof = to_prenex_normal_form(Formula.parse('~~(~Ax[Ey[R(x,y)]]&~Ax[Ey[x=y]])'))`  

`In [11]: formula`  

`Out [11]: Ez1[Az2[Ez14[Az15[~~(~R(z1,z2)&~z14=z15)]]]]`  

`In [12]: proof`  

`Out [12]: Proof of ((~~(~Ax[Ey[R(x,y)]]&~Ax[Ey[x=y]])->Ez1[Az2[Ez14[Az15[~~(~R(z1,z2)&~z14=z15)]]]])&`  
...
`In [14]: formula = Formula.parse('(R(x)|R(y))')`  

`In [15]: to_implies_not(formula)`  

`Out [15]: (~R(x)->R(y))`   

`In [16]: formula = Formula.parse('Ax[Ey[((P(x,y)&Q(y))->R(x))]]')`  
...
`In [1]: from mltp.propositions import *`  

`In [2]: Formula.parse('(p->q)').frege()`  

`Out [2]: `  
`├──┬─ q`  
`   └─ p`  

`In [3]: Formula.parse('((~p->q)->~(p->~q))').frege()`  
...
`In [4]: list(all_models(('p', 'q')))`  

`Out [4]: [{'q': False, 'p': False}, {'q': False, 'p': True}, {'q': True, 'p': False}, {'q': True, 'p': True}]`   
...
`In [5]: formula = Formula.parse('~(q&p)')`  

`In [6]: print_truth_table(formula)`  
...
`In [7]: formula = Formula.parse('(p->q)')`  

`In [8]: model = {'p': True, 'q': False}`  

`In [9]: prove_in_model_full(formula, model)`  

`Out [9]: Proof of ['p', '~q'] ==> '~(p->q)' via inference rules:`  
`  [] ==> '~F'`  
`  ...`  
`  [] ==> '((~q->~p)->(p->q))'`  
`Lines:`  
